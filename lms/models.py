...
class Subscription(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="subscribed_products"
    )
...
class Lesson(models.Model):
    ...
    video = models.FileField(
        upload_to="videos/", default=None, blank=True, null=True, verbose_name="Видео"
    )
        ...
        for user in product.subscriptions.all():
            if not sorted_groups:
                ...
                if (
                    len(sorted_groups) == 1
                    or abs(
                        max(sorted_groups, key=lambda x: x.users.count()).users.count()
                        - group.users.count()
                    )
                    <= 1
                ):
